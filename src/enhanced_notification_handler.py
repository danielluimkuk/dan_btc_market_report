...
class EnhancedNotificationHandler:
    ...
    def _generate_btc_indicators_html(self, indicators: Dict, signal_conditions: Dict, ema_200: float,
                                      price: float) -> str:
        """Generate enhanced BTC indicators HTML with 2 decimal MVRV + Mining Cost - FIXED VERSION"""
        try:
            ...
            # 🎯 FIXED: Mining Cost indicators with proper error handling
            ...
            # Format mining cost display with error handling
            mining_cost_display = "N/A"
            if mining_cost != 'N/A':
                try:
                    if isinstance(mining_cost, (int, float)) and mining_cost > 0:
                        mining_cost_display = f"${mining_cost:,.0f}"
                        if mining_cost_date != 'N/A' and str(mining_cost_date) != 'Unknown':
                            mining_cost_display += f" ({mining_cost_date})"
                    else:
                        mining_cost_display = "N/A"
                except (ValueError, TypeError) as e:
                    logging.warning(f"Error formatting mining cost: {e}")
                    mining_cost_display = "N/A"

            # Format price/cost ratio with status - FIXED with your new ranges
            ...
            if price_cost_ratio != 'N/A':
                try:
                    ...
                    if isinstance(price_cost_ratio, str):
                        ratio_value = float(price_cost_ratio)
                    else:
                        ratio_value = float(price_cost_ratio)

                    # 🎯 YOUR NEW RANGES AND EMOJIS
                    if ratio_value < 0.9:
                        ratio_status = "🟢"
                    ...
                    ratio_display = f"{ratio_value:.2f} {ratio_status}"
                    mining_cost_signal_text = f'''<div style="font-size: 11px; color: #666; margin-top: 8px; padding: 8px; background: #f8f9fa; border-radius: 4px;">
                        Price/Cost Ratio: {ratio_value:.2f} ({ratio_message})
                    </div>'''

                except (ValueError, TypeError) as e:
                    logging.warning(f"Error processing price/cost ratio: {e}")
                    ratio_display = "N/A"

            # Generate the HTML with error handling
            return f"""
            <div class="indicator">
                <span>MVRV:</span>
            ...
            logging.error(f"Error generating BTC indicators HTML: {str(e)}")
            # Return basic indicators without mining cost if there's an error
            return f"""
            <div class="indicator">
                <span>MVRV:</span>
                <span class="indicator-value">{signal_conditions.get('mvrv', {}).get('value', 2.1):.2f}</span>
            </div>
            <div class="indicator">
                <span>Weekly RSI:</span>
                <span class="indicator-value">{signal_conditions.get('rsi', {}).get('value', 65):.1f}</span>
            </div>
            <div class="indicator">
                <span>EMA 200:</span>
                <span class="indicator-value">${ema_200:,.2f}</span>
            ...
            <div class="indicator">
                <span>Price vs EMA:</span>
                <span class="indicator-value">{((price - ema_200) / ema_200 * 100) if ema_200 > 0 else 0:+.1f}%</span>
            </div>
            <div style="color: #dc3545; font-size: 12px; margin-top: 8px;">
                ⚠️ Mining cost data temporarily unavailable
            </div>
            """

    def _generate_btc_signal_boxes_html(self, signal_analysis: Dict) -> str:
        """Generate BTC signal boxes including mining cost signal - FIXED VERSION"""
        try:
            signal_status = signal_analysis.get('signal_status', {})

            # Get mining cost ratio for additional signal with error handling
            indicators = signal_analysis.get('indicators', {})
            price_cost_ratio = indicators.get('price_cost_ratio', 'N/A')

            # Generate mining cost signal with proper error handling
            mining_cost_signal = ""
            ...
            if price_cost_ratio != 'N/A':
                try:
                    ...
                    if ratio_value < 0.9:
                        ...
                        signal_text = f"🟢 Mining Cost Signal: {ratio_value:.2f} (Trading below production cost - strong value opportunity)"
                    elif 0.9 <= ratio_value <= 2.5:
                        signal_class = "hold-signal"
                        signal_text = f"🟡 Mining Cost Signal: {ratio_value:.2f} (Normal trading range of 0.9-2.5)"
                    else:  # > 2.5
                        signal_class = "sell-signal"
                        signal_text = f"🔴 Mining Cost Signal: {ratio_value:.2f} (High premium - consider profit taking)"

                    mining_cost_signal = f"""
                    <div class="signal-box {signal_class}" style="margin-top: 10px;">
                        <div class="signal-title" style="font-size: 14px;">{signal_text}</div>
                    </div>
                    """

                except (ValueError, TypeError) as e:
                    logging.warning(f"Error processing mining cost signal: {e}")
                    mining_cost_signal = f"""
                    <div class="signal-box hold-signal" style="margin-top: 10px;">
                        <div class="signal-title" style="font-size: 14px;">⚠️ Mining Cost Signal: Data temporarily unavailable</div>
                    </div>
                    """
            ...
            logging.error(f"Error generating BTC signal boxes: {str(e)}")
            # Return basic signal box if there's an error
            return """
            <div class="signal-box hold-signal">
                <div class="signal-title">🟡 HOLD SIGNAL 📊</div>
                <div class="signal-subtitle">Monitor Position</div>
